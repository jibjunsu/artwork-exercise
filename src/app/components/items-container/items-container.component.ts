...
import {
  Artwork,
  ArtWorkRequestOption,
  ArtWorkResponse,
} from '../../models/art-work.model';
  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  ngOnInit(): void {
    this.items$ = this.artWorkSubject$.pipe(
      ...
      switchMap((option: ArtWorkRequestOption) =>
        this.artWorkService.get$(option).pipe(share())
      ),
  ...
  setStyleFilterOption = (list: Artwork[]) => {
    ...
    this.styleOption = Object.entries(styleMap).map(([key, value]) => ({
      label: `${key} (${value})`,
      value: `${key}`,
    }));
